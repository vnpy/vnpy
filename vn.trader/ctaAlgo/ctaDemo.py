...
class OrderManagementDemo(CtaTemplate):
    ...
    def onTick(self, tick):
        ...
        if self.lastOrder != None and self.lastOrder.status == u'未成交':
            ...
            self.sendOrder(self.orderType, tick.lastprice - 10, 1)
            self.lastOrder = None
...
class TalibDoubleSmaDemo(CtaTemplate):
    ...
    def onBar(self, bar):
        ...
        fastSMA = ta.SMA(np.array(self.listClose), 2)
        slowSMA = ta.SMA(np.array(self.listClose), 3)

        print fastSMA, fastSMA[-1]
        # 金叉和死叉的条件是互斥
        # 所有的委托均以K线收盘价委托（这里有一个实盘中无法成交的风险，考虑添加对模拟市价单类型的支持）
