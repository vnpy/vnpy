...
class Alpha101(AlphaDataset):
    ...
    def __init__(
        ...
        # Alpha7
        self.add_feature("alpha7", "quesval2(ts_mean(volume, 20), volume, (-1 * ts_rank(abs(close - ts_delay(close, 7)), 60)) * sign(ts_delta(close, 7)), (-1) * sign(volume))")

        # Alpha8
        self.add_feature("alpha8", f"-1 * cs_rank(((ts_sum(open, 5) * ts_sum({returns_expr}, 5)) - ts_delay((ts_sum(open, 5) * ts_sum({returns_expr}, 5)), 10)))")

        # Alpha9 p
        self.add_feature("alpha9", "quesval(0, ts_min(ts_delta(close, 1), 5), ts_delta(close, 1), quesval(0, ts_max(ts_delta(close, 1), 5), ts_delta(close, 1), (-1 * ts_delta(close, 1))))")

        # Alpha10 p
        self.add_feature("alpha10", "cs_rank(quesval(0, ts_min(ts_delta(close, 1), 4), ts_delta(close, 1), quesval(0, ts_max(ts_delta(close, 1), 4), ts_delta(close, 1), (-1 * ts_delta(close, 1)))))")
        ...
        # Alpha19
        self.add_feature("alpha19", f"sign(ts_delta(close, 7) + (close - ts_delay(close, 7))) * (sign(volume) + cs_rank(sign(volume) + ts_sum({returns_expr}, 250)))")
        ...
        # Alpha23
        self.add_feature("alpha23", "quesval2(ts_mean(high, 20), high, -1 * ts_delta(high, 2), sign(volume) - sign(volume))")

        # Alpha24
        self.add_feature("alpha24", "quesval(0.05, ts_delta(ts_sum(close, 100) / 100, 100) / ts_delay(close, 100), (-1 * (close - ts_min(close, 100))), (-1 * ts_delta(close, 3)))")
        ...
        # Alpha29
        self.add_feature("alpha29", f"ts_min(cs_rank(cs_rank(cs_scale(log(ts_sum(ts_min(cs_rank(cs_rank((-1 * cs_rank(ts_delta((close - 1), 5))))), 2), 1))))), 5) + ts_rank(ts_delay((-1 * {returns_expr}), 6), 5)")
        ...
        # Alpha34 n
        self.add_feature("alpha34", f"cs_rank((sign(volume) - cs_rank(ts_std({returns_expr}, 2) / ts_std({returns_expr}, 5))) + (sign(volume) - cs_rank(ts_delta(close, 1))))")
        ...
        # Alpha39
        self.add_feature("alpha39", f"((-1) * cs_rank((ts_delta(close, 7) * (sign(volume) - cs_rank(ts_decay_linear((volume / ts_mean(volume, 20)), 9)))))) * (sign(volume) + cs_rank(ts_sum({returns_expr}, 250)))")
        ...
        # Alpha48
        self.add_feature("alpha48", "(ts_corr(ts_delta(close, 1), ts_delay(ts_delay(close, 1), 1), 250) * ts_delta(close, 1)) / close / ts_sum(pow1((ts_delta(close, 1) / ts_delay(close, 1)), 2), 250)")
        ...
        # Alpha56
        self.add_feature("alpha56", f"(-1) * (cs_rank(ts_sum({returns_expr}, 10) / ts_sum(ts_sum({returns_expr}, 2), 3)) * cs_rank(({returns_expr}) * ts_cap(vwap, volume)))")
        ...
        # Alpha63
        self.add_feature("alpha63", "cs_rank(ts_decay_linear(ts_delta(close, 2), 8) - cs_rank(ts_decay_linear(ts_corr(((vwap * 0.318108) + (open * (1 - 0.318108))), ts_sum(ts_mean(volume, 180), 37), 14), 12))) * -1")
        ...
        # Alpha68
        self.add_feature("alpha68", "cast_to_int((ts_rank(ts_corr(cs_rank(high), cs_rank(ts_mean(volume, 15)), 9), 14) < cs_rank(ts_delta((close * 0.518371 + low * (1 - 0.518371)), 1))))")

        # Alpha69
        self.add_feature("alpha69", "cs_rank(ts_max(ts_delta(vwap, 3), 5)) * -1")

        # Alpha70 n
        self.add_feature("alpha70", "cs_rank(ts_delta(vwap, 1.29456)) * -1")

        # Alpha71
        self.add_feature("alpha71", "ts_rank(ts_decay_linear(ts_corr(ts_rank(close, 3), ts_rank(ts_mean(volume, 180), 12), 18), 4), 16)")

        # Alpha72
        self.add_feature("alpha72", "cs_rank(ts_decay_linear(ts_corr((high + low) / 2, ts_mean(volume, 40), 9), 10)) / cs_rank(ts_decay_linear(ts_corr(ts_rank(vwap, 4), ts_rank(volume, 18), 7), 3))")

        # Alpha73
        self.add_feature("alpha73", "cs_rank(ts_decay_linear(ts_delta(vwap, 5), 3)) ")
        ...
        # Alpha76
        self.add_feature("alpha76", "cs_rank(ts_decay_linear(ts_delta(vwap, 1), 12))")

        # Alpha77
        self.add_feature("alpha77", "cs_rank(ts_decay_linear((((high + low) / 2 + high) - (vwap + high)), 20))")

        # Alpha78
        self.add_feature("alpha78", "cs_rank(ts_corr(ts_sum((low * 0.352233) + (vwap * (1 - 0.352233)), 20), ts_sum(ts_mean(volume, 40), 20), 7))")

        # Alpha79
        self.add_feature("alpha79", "quesval2(cs_rank(ts_delta(close * 0.60733 + open * 0.39267, 1)), cs_rank(ts_corr(ts_rank(vwap, 4), ts_rank(ts_mean(volume, 150), 9), 15)), -1 * sign(volume), 0 * sign(volume))")

        # Alpha80
        self.add_feature("alpha80", "cs_rank(sign(ts_delta(open * 0.868128 + high * 0.131872, 4))) * ts_rank(ts_corr(high, ts_mean(volume, 10), 5), 6) * -1")

        # Alpha81
        self.add_feature("alpha81", "ts_corr(vwap, ts_sum(ts_mean(volume, 10), 50), 8)")

        # Alpha82
        self.add_feature("alpha82", "ts_rank(ts_decay_linear(ts_corr(volume, open, 17), 7), 13)")
        ...
        # Alpha87 n
        self.add_feature("alpha87", "cs_rank(ts_decay_linear(ts_delta(close * 0.369701 + vwap * 0.630299, 2), 3)) * ts_rank(ts_decay_linear(abs(ts_corr(ts_mean(volume, 81), close, 13)), 5), 14)")

        # Alpha88
        self.add_feature("alpha88", "cs_rank(ts_decay_linear((cs_rank(open) + cs_rank(low)) - (cs_rank(high) + cs_rank(close)), 8))")
        ...
        # Alpha92
        self.add_feature("alpha92", "ts_rank(ts_decay_linear(quesval2(((high + low) / 2 + close), (low + open), sign(volume), 0 * sign(volume)), 15), 19)")
        ...
        # Alpha96
        self.add_feature("alpha96", "ts_rank(ts_decay_linear(ts_corr(cs_rank(vwap), cs_rank(volume), 4), 4), 8) * -1")

        # Alpha97
        self.add_feature("alpha97", "cs_rank(ts_decay_linear(ts_delta(low * 0.721001 + vwap * 0.278999, 3), 20)) * -1")
