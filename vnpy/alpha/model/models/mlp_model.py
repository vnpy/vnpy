...
class MlpModel(AlphaModel):
    ...
    def fit(
        ...
        for step in range(1, self.n_epochs + 1):
            ...
            if early_stop_count >= self.early_stop_rounds:
                logger.info("Early stopping condition met, training finished")
                break
    ...
    def _evaluate_step(
        ...
        if loss_val < best_valid_score:
            logger.info(f"\tValidation set loss decreased from {best_valid_score:.6f} to {loss_val:.6f}")
    ...
    def detail(self) -> pd.DataFrame | None:        # type: ignore
        ...
        """
        if not self.fitted:
            logger.info("Model has not been trained yet, cannot display detailed information")
            return None
