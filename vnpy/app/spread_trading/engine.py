...
class SpreadEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.algoEngine = StAlgoEngine(self.dataEngine, main_engine, event_engine)
...
class StAlgoEngine(object):
    ...
    def buy(self, vtSymbol, price, volume, payup=0):
        """Long-Open"""
        l = self.sendOrder(vtSymbol, Direction.LONG, Offset.OPEN, price, volume, payup)
        return l

    #----------------------------------------------------------------------
    def sell(self, vtSymbol, price, volume, payup=0):
        """Short-Close"""
        l = self.sendOrder(vtSymbol, Direction.SHORT, Offset.CLOSE, price, volume, payup)
        return l

    #----------------------------------------------------------------------
    def short(self, vtSymbol, price, volume, payup=0):
        """Short-Open"""
        l = self.sendOrder(vtSymbol, Direction.SHORT, Offset.OPEN, price, volume, payup)
        return l

    #----------------------------------------------------------------------
    def cover(self, vtSymbol, price, volume, payup=0):
        """Long-Close"""
        l = self.sendOrder(vtSymbol, Direction.LONG, Offset.CLOSE, price, volume, payup)
        return l
