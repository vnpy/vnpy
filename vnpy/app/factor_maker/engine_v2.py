...
class FactorEngine(BaseEngine):
    ...
    def on_bars(self, dt: datetime, bars: Dict[str, BarData]) -> None:
        ...
        # 3. Broadcast the latest factor values (from cache)
        if self.latest_calculated_factors_cache:
            # Create a copy for the event to prevent modification by subscribers
            event_data = {k: v.clone() if isinstance(v, pl.DataFrame) else v 
                          for k,v in self.latest_calculated_factors_cache.items()}
