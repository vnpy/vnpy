...
class ArbAlgo(AlgoTemplate):
    ...
    default_setting = {
        ...
        "is_close": 0,  # True 1 只平仓 False 0 持续反向套利
    }
    ...
    def __init__(
        ...
        self.is_close = setting["is_close"]
    ...
    def on_trade(self, trade: TradeData):
        ...
        if trade.vt_symbol == self.active_vt_symbol:
            ...
            # if (self.is_close == 1 and self.active_pos_btc == 0 and self.passive_pos_btc == 0):
    ...
    def on_timer(self):
        ...
        print(f"++++is close {self.is_close} , is_close == 1 {self.is_close == 1}++++")
        ...
        print(f"主动退仓位 {self.active_pos_btc}")
        print(f"被动腿仓位 {self.passive_pos_btc}")
        ...
        #
        # if (self.is_close == True):  # 平仓后不做继续反向买入操作
        #     pass
        ...
        if spread_bid_price > self.spread_up:
            ...
            if self.active_pos_btc > -self.max_pos_btc:
                ...
                #
                # self.write_log(f"卖出平仓测试---- {self.active_pos}  volume {volume} 差值:  {self.active_pos - volume}")
                # if self.is_close == 1 and self.active_pos - volume < 0 and self.active_pos != 0:
                #     self.write_log(f"仅仅平仓，执行卖出操作， 卖出volume {-self.active_pos}")
        ...
        # Buy condition
            ...
            if self.active_pos_btc < self.max_pos_btc:
                ...
                # if self.active_pos_btc == 0 and self.is_close == 1:
                ...
                # self.write_log(f"买入平仓测试---- {self.active_pos}  volume {volume} 差值:  {self.active_pos - volume}")
                #
                # if self.is_close == 1 and self.active_pos + volume > 0 and self.active_pos != 0:
