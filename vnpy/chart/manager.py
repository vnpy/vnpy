...
class BarManager:
    ...
    def update_history(self, history: List[BarData]) -> None:
        ...
        self._datetime_index_map: dict = dict(zip(dt_list, ix_list))
        self._index_datetime_map: dict = dict(zip(ix_list, dt_list))
    ...
    def get_price_range(self, min_ix: float = None, max_ix: float = None) -> Tuple[float, float]:
        ...
        if not min_ix:
            ...
            max_ix: int = min(max_ix, self.get_count())

        buf: tuple = self._price_ranges.get((min_ix, max_ix), None)
        if buf:
            return buf

        bar_list: list = list(self._bars.values())[min_ix:max_ix + 1]
        first_bar: BarData = bar_list[0]
        max_price: float = first_bar.high_price
        min_price: float = first_bar.low_price

        for bar in bar_list[1:]:
            max_price: float = max(max_price, bar.high_price)
            min_price: float = min(min_price, bar.low_price)
