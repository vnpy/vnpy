...
class CoinbaseRestApi(object):
    ...
    def processReq(self, req, i):
        ...
        header["CB-ACCESS-SIGN"] = self.generateSignature(
            method, path, timestamp, params, body=p.body
        )
        ...
        if postdict:
            resp = session.request(
                method, url, headers=header, params=params, data=json.dumps(postdict)
            )
        else:
...
class CoinbaseWebsocketApi(object):
    ...
    def start(self):
        ...
        self.ws = websocket.create_connection(
            WEBSOCKET_HOST, sslopt={"cert_reqs": ssl.CERT_NONE}
        )
