            ...
            for function_name, d in self.callbacks.items():
                ...
                for name, type_ in d.items():
                    if type_ == "unsigned int":
                        ...
                        if name == "UserNo":
                            args_list.append("string userNo")
                        else:
            ...
            for function_name, d in self.functions.items():
                ...
                for name, type_ in d.items():
                    if type_ == "unsigned int":
                        ...
                        if name == "UserNo":
                            args_list.append(f"string userNo")
    ...
    def generate_source_function(self):
        ...
        with open(filename, "w") as f:
            for name, d in self.functions.items():
                ...
                if length == 1:
                    f.write(f"int {self.class_name}::{req_name}(unsigned int request)\n")
                    f.write("{\n")
                    f.write(f"\tint i = this->api->{name}(request);\n")
                    f.write("\treturn i;\n")
                    f.write("};\n\n")
                else:
                    type_ = list(d.values())[2]
                    f.write(
                        f"int {self.class_name}::{req_name}(string userNo, unsigned int nRequestID, const dict &req)\n")
                    f.write("{\n")
                    f.write(f"\t{type_} myreq = {type_}();\n")
                    f.write("\tmemset(&myreq, 0, sizeof(myreq));\n")

                    #print(self.structs.get(type_))
                    
                    struct_fields = self.structs[type_]
                    for struct_field, struct_type in struct_fields.items():
                        if struct_type == "string":
                            line = f"\tgetString(req, \"{struct_field}\", myreq.{struct_field});\n"
                        #elif struct_type == "unsigned int":                            
                        #    line = f"\tgetUnsignedInt(req, \"{struct_field}\", myreq.{struct_field});\n"
                        else:
                            print(struct_fields)
                            line = f"\tget{struct_type.capitalize()}(req, \"{struct_field}\", &myreq.{struct_field});\n"
                        f.write(line)

                    f.write(f"\tint i = this->api->{name}((char*)userNo.c_str(), nRequestID,  &myreq);\n")
                    f.write("\treturn i;\n")
                    f.write("};\n\n")
            ...
            for name, d in self.callbacks.items():
                ...
                for field, type_ in d.items():
                    if type_ == "unsigned int":
                        ...
                        if field == "UserNo":
                            args.append(f"string userNo")
                            bind_args.append("userNo")
                        else:
                            args.append(f"string {name}")
                            bind_args.append(f"{field}")
                    else:
...
if __name__ == "__main__":
    #md_generator = ApiGenerator("../include/es/TapQuoteAPI.h", "tap", "md", "MdApi")
    #md_generator.run()

    td_generator = ApiGenerator("../include/es/EsTradeAPI.h", "tap", "td", "TdApi")
    td_generator.run()
