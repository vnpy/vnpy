...
class Generator:
    ...
    def run(self):
        ...
        self.f_define = open(f"{self.prefix}_constant_base_model_md.py", "w")
        self.f_typedef = open(f"{self.prefix}_typedef_base_model_md.py", "w")
        self.f_struct = open(f"{self.prefix}_struct_base_model_md.py", "w")
    ...
    def load_struct(self):
        """加载Struct"""
        module_name = "oes_struct_base_model_md"
    ...
    def process_line(self, line: str):
        ...
        elif "=" in line and "MDS" in line and "=>" not in line:
    ...
    def process_struct_end(self, line: str):
        ...
        if len(words) == 1 and "}" not in words:
            name = words[0]
            type_ = "dict"
            if name == "#endif" or type_ == "#ifdef":
                return

            if name == "qryFilter" or name == "securityCodeList[1]" or name == "qryItems[1]":
                return

            if name == "MdsQryStockStaticInfoFilterT":
                type_ = "MdsQryStockStaticInfoFilterT"
                name = "qryFilter"

            elif name == "MdsQrySnapshotListFilterT":
                type_ = "MdsQrySnapshotListFilterT"
                name = "qryFilter"
            elif name == "MdsQryOptionStaticInfoFilterT":
                type_ = "MdsQryOptionStaticInfoFilterT"
                name = "qryFilter"
            elif name == "MdsQrySecurityCodeEntryT":
                type_ = "MdsQrySecurityCodeEntryT"
                name = "securityCodeList"
            elif name == "MdsOptionStaticInfoT":
                type_ = "MdsOptionStaticInfoT"
                name = "qryItems"

            if type_.startswith("Mds"):
                type_ = "dict"
