    ...
    if line.count("*") == 1:
        ...
        if "is_last" in line:
            on_line = (
                "virtual void on"
                + cbName[2:]
                + "(dict data, dict error, int reqid, bool last) {};\n"
            )
        else:
            on_line = "virtual void on" + cbName[2:] + "(dict data, dict error) {};\n"
    elif line.count("*") == 0:
    ...
    if line.count("*") == 1:
        ...
        if "int" in line:
            on_line = (
                "virtual void on"
                + cbName[2:]
                + "(dict data, dict error, int id, bool last)\n"
            )
            override_line = '("on' + cbName[2:] + '")(data, error, id, last);\n'
        else:
            on_line = "virtual void on" + cbName[2:] + "(dict data, dict error)\n"
...
def createTask(cbName, cbArgsTypeList, cbArgsValueList, orignalLine):
    # 从回调函数生成任务对象，并放入队列
    funcline = orignalLine.replace(
        "            virtual void ", "void " + apiName + "::"
    )
...
def createProcess(cbName, cbArgsTypeList, cbArgsValueList):
    ...
    for i, type_ in enumerate(cbArgsTypeList):
        if "XTPRI" in type_:
            ...
            fprocess.write("    {\n")
            fprocess.write(
                "        " + type_ + " *task_data = (" + type_ + "*) task->task_data;\n"
            )
...
def createFunction(fcName, fcArgsTypeList, fcArgsValueList):
    ...
    ffunction.write(
        "int TraderApi::" + fcName[0].lower() + fcName[1:] + "(dict req, int reqid)\n"
    )
    ffunction.write("{\n")
