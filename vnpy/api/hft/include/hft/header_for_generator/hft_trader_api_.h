...
class TraderSpi {
   public:
     ...
     * @param is_last       是否是本次请求的最后一笔响应
     */
    virtual void OnQueryETFRsp(ETFDetail* etf_detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param is_last               是否是本次请求的最后一笔响应
     */
    virtual void OnQueryETFStockRsp(ETFStockDetail* etf_stock_detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param request_id    对应请求时传入的序列号
     */
    virtual void OnQueryIPOStockRsp(IPOStockDetail* detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param pos_str       本次查询最后一条记录的定位串，用于下一次查询
     */
    virtual void OnQueryIPODistributionRsp(IPODistributionDetail* detail, ErrorInfo* error_info, int request_id, bool is_last, const char* pos_str) {}
     ...
     * @param pos_str       本次查询最后一条记录的定位串，用于下一次查询
     */
    virtual void OnQueryIPOLotteryRsp(IPOLotteryDetail* detail, ErrorInfo* error_info, int request_id, bool is_last, const char* pos_str) {}
    ...
    */
     ...
     * @param is_last           是否是本次请求的最后一笔响应
     */
    virtual void OnQueryCreditSecuritySellQtyRsp(CreditSecuritySellQtyRsp* rsp, ErrorInfo* error_info, int request_id, bool is_last) {
    }
     ...
     * @param request_id    请求序列号，用于匹配响应，由用户自定义，非0
     */
    virtual void OnQuerySecuidRightRsp(const char* market, int16_t secuid_right, ErrorInfo* error_info, bool sign_flag, int request_id) {}
     ...
     * @param is_last       是否是本次请求的最后一笔响应
     */
    virtual void OnQueryHKStockRsp(HKStockDetail* detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param is_last       是否是本次请求的最后一笔响应
     */
    virtual void OnQueryHKMinPriceUnitRsp(HKMinPriceUnitDetail* detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param is_last       是否是本次请求的最后一笔响应
     */
    virtual void OnQueryLockSecurityDetailRsp(LockSecurityDetail* detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param request_id    请求序列号，用于匹配响应，由用户自定义，非0
     */
    virtual void OnExtendLockSecurityRsp(int32_t apply_sno, int32_t apply_date, ErrorInfo* error_info, int request_id) {}
     ...
     * @param is_last       是否是本次请求的最后一笔响应
     */
    virtual void OnQueryLockSecurityExtensionRsp(LockSecurityExtensionDetail* detail, ErrorInfo* error_info, int request_id, bool is_last) {}
     ...
     * @param request_id    对应请求时传入的序列号
     */
    virtual void OnCancelExtendLockSecurityRsp(int apply_date, const char* apply_sno, const char* cancel_sno, ErrorInfo* error_info, int request_id) {}
...
class HFT_TRADER_EXPORT TraderApi {
   public:
     ...
     */
    virtual int Login(const char* svr_ip, int svr_port, const AccountInfo* account_info, const char* terminal_info) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryOrder(const char* order_id, int request_id, const char* market = NULL) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryOrders(const char* pos_str, int query_num, int request_id, int query_flag = 0, const char* market = NULL) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryTradeByOrderId(const char* order_id, int request_id, const char* market = NULL) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryTrades(const char* pos_str, int query_num, int request_id, const char* market = NULL, bool filter_etf = false) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryETFTrades(const char* pos_str, int query_num, int request_id, const char* market = NULL) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryPositions(const char* pos_str, int query_num, int request_id, const char* market = NULL) = 0;
     ...
     * @return                  成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int TransferFundInAndOut(int transfer_side, int64_t transfer_value, int request_id, const char* market = NULL) = 0;
     ...
     * @return                  成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int TransferFundBetweenSecuid(const char* fund_out_market, const char* fund_in_market, int64_t transfer_value, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryMaxOrderQty(const char* symbol, int16_t order_type, int16_t side, int64_t price, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryIPODistribution(int start_date, int end_date, const char* pos_str, int query_num, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryIPOLottery(int start_date, int end_date, const char* pos_str, int query_num, int request_id) = 0;
    ...
    * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
    */
    virtual int QuerySecurityBaseInfo(int request_id, const char* symbol = NULL) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int CreditPayBackByOrder(const char* order_id, int order_date, int64_t back_amt, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryCreditStock(const char* pos_str, int query_num, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryCreditMortgageHold(const char* pos_str, int query_num, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryCreditFinance(const char* pos_str, int query_num, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryCreditShortsell(const char* pos_str, int query_num, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryCreditRepayStock(const char* market, const char* code, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryCreditSecuritySellQty(const char* market, const char* code, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QuerySecuidRight(const char* market, int16_t secuid_right, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryHKTradeCalendar(int start_date, int end_date, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryLockSecurityDetail(int begin_date, int end_date, int request_id, const char* market = NULL, const char* code = NULL) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int QueryLockSecurityExtension(int begin_date, int end_date, int request_id) = 0;
     ...
     * @return              成功返回0，失败返回错误码，通过GetApiLastError获取错误信息
     */
    virtual int CancelExtendLockSecurity(int apply_date, const char* apply_sno, int request_id) = 0;
