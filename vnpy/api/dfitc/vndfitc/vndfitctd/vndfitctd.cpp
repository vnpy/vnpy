...
void TdApi::createDFITCSECTraderApi(string pszFlowPath, string protocol)
{
	this->api = DFITCSECTraderApi::CreateDFITCSECTraderApi(pszFlowPath.c_str());
	this->api->Init(protocol.c_str(), this);

	int a = this->api->Init(protocol.c_str(), this);
	cout << "前置" << protocol.c_str() << endl;
	if (a == 0)
	{
		cout << "TD服务端前置机链接成功" << endl;
	}
	else
	{
		cout << "TD服务端前置机链接失败,错误原因:" << a << endl;
	}
};
...
void TdApi::init()
{
	...
	//this->api->Init(pszFlowPath, this);
};
