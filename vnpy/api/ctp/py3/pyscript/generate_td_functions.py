...
def processCallBack(line):
    ...
    if "OnRspError" in cbName:
        on_line = (
            "virtual void on" + cbName[2:] + "(dict error, int id, bool last) {};\n"
        )
...
def createProcess(cbName, cbArgsTypeList, cbArgsValueList):
    ...
    for i, type_ in enumerate(cbArgsTypeList):
        if "RspInfoField" in type_:
            ...
            for key in struct.keys():
                ...
                if struct[key] == "string":
                    fprocess.write(
                        ...
                        + 'error["'
                        + key
                        + '"] = boost::locale::conv::to_utf<char>(task_error.'
                        + key
                        + ', std::string("GB2312"));\n'
                    )
                else:
            ...
            fprocess.write(
                "\t" + type_ + " task_data = any_cast<" + type_ + ">(task.task_data);\n"
            )
            ...
            for key in struct.keys():
                ...
                if struct[key] == "string":
                    fprocess.write(
                        ...
                        + 'data["'
                        + key
                        + '"] = boost::locale::conv::to_utf<char>(task_data.'
                        + key
                        + ', std::string("GB2312"));\n'
                    )
                else:
