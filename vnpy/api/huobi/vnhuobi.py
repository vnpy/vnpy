...
class Huobi_TradeApi(object):
    ...
    def processRequest(self, req):
        ...
        try:
            ...
            if method in [FUNCTIONCODE_GET_SYMBOS_HUOBI]:
                data = self.http_get_request( url = url , params = kwargs )
    ...
    def get_symbols(self):
        ...
        kwargs = {}
        return self.sendRequest( url , FUNCTIONCODE_GET_SYMBOS_HUOBI , self.onAllSymbols , kwargs = kwargs , optional = None)
    ...
    def get_accounts(self):
        ...
        return self.sendRequest( url , FUNCTIONCODE_GET_ACCOUNTS_HUOBI , self.onGetAccounts , kwargs = kwargs , optional = None)
    ...
    # 创建并执行订单
        ...
        """
    #----------------------------------------------------------------------
    def spotTrade(self, symbol , amount, _type , price = 0 ,source = "api"):
        try:
            if not self.account_id:
                try:
                    self.get_accounts()
...
class Huobi_DataApi(object):
    ...
    def run(self):
        """连续运行"""
        while self.active:
            for url, callback ,params in self.taskList:
                try:
                    callback(self.http_get_request(url, params , add_to_headers = None))
                except Exception, e:
                    print e
