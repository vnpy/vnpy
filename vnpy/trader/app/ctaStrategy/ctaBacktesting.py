...
class BacktestingEngine(object):
    ...
    def __init__(self):
        ...
        self.stopOrderCount = (
            0
        )  # 编号计数：stopOrderID = STOPORDERPREFIX + str(stopOrderCount)
    ...
    def loadHistoryData(self):
        ...
        self.dbClient = pymongo.MongoClient(
            globalSetting["mongoHost"], globalSetting["mongoPort"]
        )
        ...
        if self.hdsClient:
            initCursor = self.hdsClient.loadHistoryData(
                self.dbName, self.symbol, self.dataStartDate, self.strategyStartDate
            )
        else:
            flt = {
                "datetime": {"$gte": self.dataStartDate, "$lt": self.strategyStartDate}
            }
        ...
        if self.hdsClient:
            self.dbCursor = self.hdsClient.loadHistoryData(
                self.dbName, self.symbol, self.strategyStartDate, self.dataEndDate
            )
        else:
    ...
    def showBacktestingResult(self):
        ...
        self.output(
            u"平均每笔滑点：\t%s" % formatNumber(d["totalSlippage"] / d["totalResult"])
        )
        self.output(
            u"平均每笔佣金：\t%s" % formatNumber(d["totalCommission"] / d["totalResult"])
        )
...
class TradingResult(object):
    ...
    # ----------------------------------------------------------------------
    def __init__(
        self, entryPrice, entryDt, exitPrice, exitDt, volume, rate, slippage, size
    ):
        """Constructor"""
        ...
        self.pnl = (
            (self.exitPrice - self.entryPrice) * volume * size
            - self.commission
            - self.slippage
        )  # 净盈亏
...
class DailyResult(object):
    ...
    def calculatePnl(self, openPosition=0, size=1, rate=0, slippage=0):
        ...
        self.positionPnl = (
            self.openPosition * (self.closePrice - self.previousClose) * size
        )
