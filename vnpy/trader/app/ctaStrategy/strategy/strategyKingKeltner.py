...
class KkStrategy(CtaTemplate):
    ...
    # 参数列表，保存了参数的名称
    paramList = ["name", "className", "author", "vtSymbol", "kkLength", "kkDev"]
    ...
    def onFiveBar(self, bar):
        ...
        # 持有多头仓位
            ...
            l = self.sell(
                self.intraTradeHigh * (1 - self.trailingPrcnt / 100),
                abs(self.pos),
                True,
            )
        ...
        # 持有空头仓位
            ...
            l = self.cover(
                self.intraTradeLow * (1 + self.trailingPrcnt / 100), abs(self.pos), True
            )
            self.orderList.extend(l)
