    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        for convertedReq in reqList:
            vtOrderID = self.mainEngine.sendOrder(
                convertedReq, contract.gatewayName
            )  # 发单
        ...
        if vtSymbol in self.tickStrategyDict:
            ...
            for so in self.workingStopOrderDict.values():
                if so.vtSymbol == vtSymbol:
                    longTriggered = (
                        so.direction == DIRECTION_LONG and tick.lastPrice >= so.price
                    )  # 多头停止单被触发
                    shortTriggered = (
                        so.direction == DIRECTION_SHORT and tick.lastPrice <= so.price
                    )  # 空头停止单被触发
                    ...
                    if longTriggered or shortTriggered:
                        ...
                        vtOrderID = self.sendOrder(
                            so.vtSymbol, so.orderType, price, so.volume, so.strategy
                        )
