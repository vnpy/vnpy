...
class CtaTemplate(object):
    ...
    def sendOrder(self, orderType, price, volume, stop=False):
        ...
        if self.trading:
            ...
            if stop:
                vtOrderIDList = self.ctaEngine.sendStopOrder(
                    self.vtSymbol, orderType, price, volume, self
                )
            else:
                vtOrderIDList = self.ctaEngine.sendOrder(
                    self.vtSymbol, orderType, price, volume, self
                )
...
class BarGenerator(object):
    ...
    def updateTick(self, tick):
        ...
        if self.lastTick:
            ...
            self.bar.volume += max(
                volumeChange, 0
            )  # 避免夜盘开盘lastTick.volume为昨日收盘数据，导致成交量变化为负的情况
