...
class RmEngine(object):
    ...
    def checkRisk(self, orderReq, gatewayName):
        ...
        if orderReq.volume > self.orderSizeLimit:
            self.writeRiskLog(
                u"单笔委托数量%s，超过限制%s" % (orderReq.volume, self.orderSizeLimit)
            )
            return False
        ...
        if self.tradeCount >= self.tradeLimit:
            self.writeRiskLog(
                u"今日总成交合约数量%s，超过限制%s" % (self.tradeCount, self.tradeLimit)
            )
            return False
        ...
        if workingOrderCount >= self.workingOrderLimit:
            self.writeRiskLog(
                u"当前活动委托数量%s，超过限制%s" % (workingOrderCount, self.workingOrderLimit)
            )
            return False

        # 检查撤单次数
        if (
            orderReq.symbol in self.orderCancelDict
            and self.orderCancelDict[orderReq.symbol] >= self.orderCancelLimit
        ):
            self.writeRiskLog(
                u"当日%s撤单次数%s，超过限制%s"
                % (
                    orderReq.symbol,
                    self.orderCancelDict[orderReq.symbol],
        ...
        if (
            ...
            and gatewayName in self.marginRatioDict
            and self.marginRatioDict[gatewayName] >= self.marginRatioLimit
        ):
            self.writeRiskLog(
                u"%s接口保证金占比%s，超过限制%s"
                % (
