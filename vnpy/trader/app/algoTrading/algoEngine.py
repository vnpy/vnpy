...
class AlgoEngine(object):
    ...
    # ----------------------------------------------------------------------
    def sendOrder(
        self, algo, vtSymbol, direction, price, volume, priceType=None, offset=None
    ):
        """发单"""
        contract = self.mainEngine.getContract(vtSymbol)
        if not contract:
    ...
    def buy(self, algo, vtSymbol, price, volume, priceType=None, offset=None):
        ...
        return self.sendOrder(
            algo, vtSymbol, DIRECTION_LONG, price, volume, priceType, offset
        )
    ...
    def sell(self, algo, vtSymbol, price, volume, priceType=None, offset=None):
        """卖出"""
        return self.sendOrder(
            algo, vtSymbol, DIRECTION_SHORT, price, volume, priceType, offset
        )
    ...
    def loadAlgoSetting(self):
        """加载算法配置"""
        l = self.mainEngine.dbQuery(
            ALGOTRADING_DB_NAME, SETTING_COLLECTION_NAME, {}, "templateName"
        )
    ...
    def deleteAlgoSetting(self, algoSetting):
        ...
        self.mainEngine.dbDelete(
            ALGOTRADING_DB_NAME, SETTING_COLLECTION_NAME, {"settingName": settingName}
        )
