...
class StopButton(QtWidgets.QPushButton):
    ...
    def stopAlgo(self):
        """停止某一算法"""
        self.algo_engine.writeLog(u'界面调用，停止算法:{}'.format(self.algoName))
        self.algo_engine.stopAlgo(self.algoName)
        self.disable()

    #----------------------------------------------------------------------
    def stopAll(self):
        """停止全部算法"""
        self.algo_engine.stopAll()
...
class AlgoStatusMonitor(QtWidgets.QTableWidget):
    ...
    def __init__(self, algoEngine, mode):
        ...
        self.algo_engine = algoEngine
        self.event_engine = algoEngine.event_engine
        self.mode = mode
    ...
    def registerEvent(self):
        ...
        self.event_engine.register(EVENT_ALGO_PARAM, self.signalParam.emit)
        self.event_engine.register(EVENT_ALGO_VAR, self.signalVar.emit)
    ...
    def addAlgo(self, algoName):
        ...
        buttonStop = StopButton(self.algo_engine, algoName)
    ...
    def processParamEvent(self, event):
        ...
        try:
            ...
            print(u'processParamEvent Exception:{}'.format(str(ex)))
            traceback.print_exc()
    #----------------------------------------------------------------------
    def processVarEvent(self, event):
        ...
        try:
            ...
            print(u'processVarEvent Exception:{}'.format(str(ex)))
            traceback.print_exc()
    #----------------------------------------------------------------------
...
class StartButton(QtWidgets.QPushButton):
    ...
    def startAlgo(self):
        """启动算法"""
        self.algo_engine.addAlgo(self.setting)
...
class DeleteButton(QtWidgets.QPushButton):
    ...
    def deleteAlgoSetting(self):
        """删除算法配置"""
        self.algo_engine.deleteAlgoSetting(self.setting)
...
class AlgoSettingMonitor(QtWidgets.QTableWidget):
    ...
    def registerEvent(self):
        ...
        self.event_engine.register(EVENT_ALGO_SETTING, self.signal.emit)
    ...
    def processEvent(self, event):
        ...
        # 新增配置行
            ...
            buttonStart = StartButton(self.algo_engine, setting)
            buttonDelete = DeleteButton(self.algo_engine, setting)
...
class AlgoManager(QtWidgets.QWidget):
    ...
    def __init__(self, algoEngine, eventEngine, parent=None):
        ...
        self.initUi()
        self.changeWidget()
        self.algo_engine.loadAlgoSetting()   # 界面初始化后，再加载算法配置
    ...
    def initUi(self):
        ...
        for templateName, widgetClass in WIDGET_DICT.items():
            widget = widgetClass(self.algo_engine)
        ...
        self.buttonStop = StopButton(self.algo_engine)
        ...
        workingMonitor = AlgoStatusMonitor(self.algo_engine, AlgoStatusMonitor.MODE_WORKING)
        workingMonitor.setFixedWidth(1500)

        historyMonitor = AlgoStatusMonitor(self.algo_engine, AlgoStatusMonitor.MODE_HISTORY)
        logMonitor = AlgoLogMonitor(self.algo_engine)
        settingMonitor = AlgoSettingMonitor(self.algo_engine)
    ...
    def addAlgoWidget(self, widgetClass):
        """添加算法控制组件 """
        w = widgetClass(self.algo_engine)
    ...
    def loadCsv(self, path):
        ...
        try:
            ...
            self.algo_engine.writeLog(u'读取CSV文件失败：\n' + msg)
            return []
    ...
    def saveSettingFromCsv(self):
        ...
        l = self.loadCsv(path)
        for setting in l:
            self.algo_engine.saveAlgoSetting(setting)
    ...
    def addAlgoFromCsv(self):
        ...
        l = self.loadCsv(path)
        for setting in l:
            self.algo_engine.addAlgo(setting)
    ...
    def startRpc(self):
        ...
        try:
            ...
            self.algo_engine.writeLog(u'请检查RPC端口，只能使用整数')
            return

        self.algo_engine.startRpc(repPort, pubPort)
