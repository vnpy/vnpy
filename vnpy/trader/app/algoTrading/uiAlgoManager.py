...
class AlgoManager(QtWidgets.QWidget):
    ...
    def initUi(self):
        ...
        workingMonitor = AlgoStatusMonitor(
            self.algoEngine, AlgoStatusMonitor.MODE_WORKING
        )
        workingMonitor.setFixedWidth(1500)

        historyMonitor = AlgoStatusMonitor(
            self.algoEngine, AlgoStatusMonitor.MODE_HISTORY
        )
    ...
    def saveSettingFromCsv(self):
        """从CSV加载配置到数据库"""
        path, fileType = QtWidgets.QFileDialog.getOpenFileName(
            self, u"加载算法配置", "", "CSV(*.csv)"
        )
        l = self.loadCsv(path)
        for setting in l:
