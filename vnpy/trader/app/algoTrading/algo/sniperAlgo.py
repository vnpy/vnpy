...
class SniperAlgo(AlgoTemplate):
    ...
    def onTick(self, tick):
        ...
        # 做多，且卖1价格小于等于执行目标价
        if self.direction == DIRECTION_LONG and tick.askPrice1 <= self.price:
            orderVolume = self.volume - self.tradedVolume
            orderVolume = min(orderVolume, tick.askVolume1)
            self.vtOrderID = self.buy(
                self.vtSymbol, self.price, orderVolume, offset=self.offset
            )

        # 做空
        elif self.direction == DIRECTION_SHORT and tick.bidPrice1 >= self.price:
            orderVolume = self.volume - self.tradedVolume
            orderVolume = min(orderVolume, tick.bidVolume1)
            self.vtOrderID = self.sell(
                self.vtSymbol, self.price, orderVolume, offset=self.offset
            )
