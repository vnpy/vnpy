...
class ArbitrageAlgo(AlgoTemplate):
    ...
    def onTimer(self):
        ...
        if spreadBidPrice > self.spread:
            self.activeOrderID = self.sell(
                self.activeVtSymbol, activeTick.bidPrice1, spreadBidVolume
            )
        elif spreadAskPrice < -self.spread:
            self.activeOrderID = self.buy(
                self.activeVtSymbol, activeTick.askPrice1, spreadAskVolume
            )
    ...
    def hedge(self):
        ...
        if self.netPos > 0:
            self.passiveOrderID = self.sell(
                self.passiveVtSymbol, tick.bidPrice5, volume
            )
        elif self.netPos < 0:
            self.passiveOrderID = self.buy(self.activeVtSymbol, tick.askPrice5, volume)
