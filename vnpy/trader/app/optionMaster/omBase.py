...
class OmOption(OmInstrument):
    ...
    def calculateOptionImpv(self):
        ...
        self.askImpv = self.calculateImpv(
            self.askPrice1, underlyingPrice, self.k, self.r, self.t, self.cp
        )
        if self.askImpv > 1:  # 正常情况下波动率不应该超过100%
            self.askImpv = 0.01  # 若超过则大概率为溢出，调整为1%

        self.bidImpv = self.calculateImpv(
            self.bidPrice1, underlyingPrice, self.k, self.r, self.t, self.cp
        )
        if self.bidImpv > 1:
            self.bidImpv = 0.01
    ...
    def calculateTheoGreeks(self):
        ...
        self.theoPrice, delta, gamma, theta, vega = self.calculateGreeks(
            underlyingPrice, self.k, self.r, self.t, self.pricingImpv, self.cp
        )
...
class OmChain(object):
    ...
    def adjustR(self):
        ...
        for n, call in enumerate(callList):
            ...
            # 如果有任意中间价为0，则忽略该PCP
            if (
                not call.underlying.midPrice
                or not put.midPrice
                or not call.midPrice
                or not call.k
                or not call.t
            ):
                continue
