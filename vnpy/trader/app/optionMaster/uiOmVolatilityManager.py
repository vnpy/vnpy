...
class VolatilityChart(pg.GraphicsWindow):
    ...
    def initUi(self):
        ...
        for chain in portfolio.chainDict.values():
            ...
            self.bidCurveDict[symbol] = chart.plot(
                pen="r", symbol="t", symbolSize=8, symbolBrush="r"
            )
            self.askCurveDict[symbol] = chart.plot(
                pen="g", symbolSize=8, symbolBrush="g"
            )
            self.pricingCurveDict[symbol] = chart.plot(
                pen="w", symbol="s", symbolSize=8, symbolBrush="w"
            )
...
class ChainVolatilityMonitor(QtWidgets.QTableWidget):
    ...
    headers = [u"代码", u"买隐波", u"定价", u"卖隐波", u"行权价", u"买隐波", u"定价", u"卖隐波", u"代码"]
    ...
    def initUi(self):
        ...
        for row, option in enumerate(chain.callDict.values()):
            ...
            cellPricing = OmCell(
                "%.1f" % (option.pricingImpv * 100), COLOR_POS, COLOR_BLACK
            )
...
class ChainVolatilityManager(QtWidgets.QWidget):
    ...
    def initUi(self):
        ...
        buttonCallIncrease = QtWidgets.QPushButton(
            u"看涨+" + ("%.1f%%" % (self.IMPV_CHANGE_STEP * 100))
        )
        buttonCallDecrease = QtWidgets.QPushButton(
            u"看涨-" + ("%.1f%%" % (self.IMPV_CHANGE_STEP * 100))
        )
        buttonPutIncrease = QtWidgets.QPushButton(
            u"看跌+" + ("%.1f%%" % (self.IMPV_CHANGE_STEP * 100))
        )
        buttonPutDecrease = QtWidgets.QPushButton(
            u"看跌-" + ("%.1f%%" % (self.IMPV_CHANGE_STEP * 100))
        )
