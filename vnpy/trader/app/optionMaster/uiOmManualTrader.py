...
class ChainMonitor(QtWidgets.QTableWidget):
    ...
    def initUi(self):
        ...
        self.horizontalHeader().setResizeMode(
            self.columnCount() - 1, QtWidgets.QHeaderView.ResizeToContents
        )
        ...
        for underlying in portfolio.underlyingDict.values():
            ...
            cellBidPrice = OmCell(
                str(underlying.bidPrice1), COLOR_BID, COLOR_BLACK, underlying
            )
            cellBidVolume = OmCell(
                str(underlying.bidVolume1), COLOR_BID, COLOR_BLACK, underlying
            )
            cellAskPrice = OmCell(
                str(underlying.askPrice1), COLOR_ASK, COLOR_BLACK, underlying
            )
            cellAskVolume = OmCell(
                str(underlying.askVolume1), COLOR_ASK, COLOR_BLACK, underlying
            )
        ...
        for chain in portfolio.chainDict.values():
            ...
            for option in chain.callDict.values():
                ...
                cellBidPrice = OmCell(
                    str(option.bidPrice1), COLOR_BID, COLOR_BLACK, option
                )
                cellBidVolume = OmCell(
                    str(option.bidVolume1), COLOR_BID, COLOR_BLACK, option
                )
                cellBidImpv = OmCell(
                    "%.1f" % (option.bidImpv * 100), COLOR_BID, COLOR_BLACK, option
                )
                cellAskPrice = OmCell(
                    str(option.askPrice1), COLOR_ASK, COLOR_BLACK, option
                )
                cellAskVolume = OmCell(
                    str(option.askVolume1), COLOR_ASK, COLOR_BLACK, option
                )
                cellAskImpv = OmCell(
                    "%.1f" % (option.askImpv * 100), COLOR_ASK, COLOR_BLACK, option
                )
    ...
    def registerEvent(self):
        ...
        for underlying in portfolio.underlyingDict.values():
            self.eventEngine.register(
                EVENT_TICK + underlying.vtSymbol, self.signalTick.emit
            )
            self.eventEngine.register(
                EVENT_TRADE + underlying.vtSymbol, self.signalTrade.emit
            )

        for chain in portfolio.chainDict.values():
            for option in chain.optionDict.values():
                self.eventEngine.register(
                    EVENT_TICK + option.vtSymbol, self.signalTick.emit
                )
                self.eventEngine.register(
                    EVENT_TRADE + option.vtSymbol, self.signalTrade.emit
                )
        ...
        if direction == DIRECTION_LONG:
            ...
            # 否则先平后开
                ...
                if instrument.shortPos:
                    self.fastTrade(
                        symbol, DIRECTION_LONG, OFFSET_CLOSE, price, instrument.shortPos
                    )
        ...
        # 做空
            ...
            if instrument.longPos >= volume:
                ...
                if instrument.longPos:
                    self.fastTrade(
                        symbol, DIRECTION_SHORT, OFFSET_CLOSE, price, instrument.longPos
                    )
