...
class ScenarioValueMonitor(QtWidgets.QTableWidget):
    ...
    def updateData(self, result, priceChangeArray, impvChangeArray):
        ...
        priceChangeHeaders = [
            ("price %s%%" % (priceChange * 100)) for priceChange in priceChangeArray
        ]
        self.setHorizontalHeaderLabels(priceChangeHeaders)

        self.setRowCount(len(impvChangeArray))
        impvChangeHeaders = [
            ("impv %s%%" % (impvChange * 100)) for impvChange in impvChangeArray
        ]
        ...
        for priceChange in priceChangeArray:
            for impvChange in impvChangeArray:
                ...
                for underlying in portfolio.underlyingDict.values():
                    portfolioPnl += (
                        underlying.midPrice * underlying.netPos * priceChange
                    )
                ...
                try:
                    for option in portfolio.optionDict.values():
                        ...
                        price, delta, gamma, theta, vega = calculateGreeks(
                            option.underlying.midPrice * (1 + priceChange),
                            option.k,
                            option.r,
                        ...
                        portfolioPnl += (
                            (price - option.theoPrice) * option.netPos * option.size
                        )
