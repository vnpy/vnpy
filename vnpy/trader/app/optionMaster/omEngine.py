...
from vnpy.trader.vtObject import VtLogData, VtSubscribeReq, VtOrderReq, VtCancelOrderReq
...
class OmEngine(object):
    ...
    def initEngine(self, fileName):
        ...
        for d in setting["chain"]:
            ...
            if not underlying:
                self.writeLog(
                    u"%s期权链的标的合约%s尚未创建，请检查配置文件" % (chainSymbol, underlyingSymbol)
                )
                continue
        ...
        self.portfolio = OmPortfolio(
            setting["name"], model, underlyingDict.values(), chainList
        )
...
class OmStrategyEngine(object):
    ...
    def callStrategyFunc(self, strategy, func, params=None):
        ...
        try:
            ...
            # 发出日志
            content = "\n".join(
                [u"策略%s触发异常已停止" % strategy.name, traceback.format_exc()]
            )
