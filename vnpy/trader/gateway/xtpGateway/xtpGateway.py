...
class XtpMdApi(QuoteApi):
    ...
    def onDepthMarketData(self, data):
        ...
        tick.bidPrice1, tick.bidPrice2, tick.bidPrice3, tick.bidPrice4, tick.bidPrice5 = data[
            "bid"
        ][
            0:5
        ]
        tick.askPrice1, tick.askPrice2, tick.askPrice3, tick.askPrice4, tick.askPrice5 = data[
            "ask"
        ][
            0:5
        ]
        tick.bidVolume1, tick.bidVolume2, tick.bidVolume3, tick.bidVolume4, tick.bidVolume5 = data[
            "bid_qty"
        ][
            0:5
        ]
        tick.askVolume1, tick.askVolume2, tick.askVolume3, tick.askVolume4, tick.askVolume5 = data[
            "ask_qty"
        ][
            0:5
        ]
    ...
    def onQueryAllTickers(self, data, error, last):
        ...
        contract.exchange = exchangeMapReverse.get(
            data["exchange_id"], EXCHANGE_UNKNOWN
        )
        ...
        contract.productClass = productClassMapReverse.get(
            data["ticker_type"], PRODUCT_UNKNOWN
        )
    ...
    def subscribe(self, subscribeReq):
        ...
        if self.loginStatus:
            self.subscribeMarketData(
                str(subscribeReq.symbol), exchangeMap[subscribeReq.exchange]
            )
...
class XtpTdApi(TraderApi):
    ...
    def onOrderEvent(self, data, error, session):
        ...
        if orderID not in self.orderDict:
            ...
            # 开平和方向
            order.direction, order.offset = sideMapReverse.get(
                data["side"], (DIRECTION_UNKNOWN, OFFSET_UNKNOWN)
            )
        ...
        if error["error_id"]:
            ...
            err.errorID = error["error_id"]
            err.errorMsg = (
                u"委托号" + str(order.orderID) + ":" + error["error_msg"].decode("gbk")
            )
    ...
    def onTradeEvent(self, data, session):
        ...
        # 开平和方向
        trade.direction, trade.offset = sideMapReverse.get(
            data["side"], (DIRECTION_UNKNOWN, OFFSET_UNKNOWN)
        )
        ...
        if (
            not order
            or order.status is STATUS_ALLTRADED
            or order.status is STATUS_CANCELLED
        ):
            return
