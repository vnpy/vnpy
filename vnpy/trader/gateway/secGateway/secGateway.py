...
] = DATA_TYPE.DFITCSEC_OT_SHBESTFRTradeLeftWithdraw
...
class SecTdApi(TdApi):
    ...
    def onStockEntrustOrderRtn(self, data):
        ...
        if newLocalID in self.orderDict:
            ...
            order.direction = directionMapReverse.get(
                data["entrustDirection"], DIRECTION_UNKNOWN
            )
    ...
    def onRspSOPQryContactInfo(self, data, error, flag):
        ...
        contract.underlyingSymbol = "-".join(
            [data["securityID"], str(data["endTradingDay"])[2:-2]]
        )
