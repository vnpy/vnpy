...
# 方向映射
directionMap = {constant.DIRECTION_LONG: "bid", constant.DIRECTION_SHORT: "ask"}
...
class RestApi(BithumbRestApi):
    ...
    def sendOrder(self, orderReq):
        ...
        self.fillLocalOrder(
            order, orderReq.symbol, orderReq.price, orderReq.volume, orderReq.direction
        )
    ...
    def onQryOrders(self, data, reqid):
        ...
        for detail in orders:
            ...
            order.tradedVolume = newTradeVolume = (
                order.totalVolume - detail["units_remaining"]
            )
            ...
            order.status = (
                constant.STATUS_ALLTRADED if detail["date_completed"] else order.status
            )
    ...
    def qryPublicTick(self, symbol="ALL"):
        ...
        if symbol.upper() == "ALL":
            ...
            self.addReq(
                "GET",
                url,
                callback=lambda data, reqid: self.onQrySinglePublicTicker(
                    symbol, data, id
                ),
            )
    ...
    def qryPublicOrderBook(self, symbol="ALL"):
        ...
        if symbol.upper() == "ALL":
            ...
            self.addReq(
                "GET",
                url,
                callback=lambda data, reqid: self.onQrySinglePublicOrderBook(
                    symbol, data, id
                ),
            )
    ...
    def qryPrivateTick(self, symbol, currency="CNY"):
        """"""
        req = {"order_currency": symbol, "payment_currency": currency}
    ...
    def parsePublicOrderBookData(self, symbol, info):
        ...
        for i in range(5):
            ...
            tick.__setattr__(
                "askVolume" + str(i + 1), float(info["asks"][i]["quantity"])
            )

        for i in range(5):
            tick.__setattr__("bidPrice" + str(i + 1), float(info["bids"][i]["price"]))
            tick.__setattr__(
                "bidVolume" + str(i + 1), float(info["bids"][i]["quantity"])
            )
