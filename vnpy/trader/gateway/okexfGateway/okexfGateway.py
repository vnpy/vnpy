...
class OkexfRestApi(RestClient):
    ...
    def cancelOrder(self, cancelOrderReq):
        ...
        req = {"instrument_id": symbol, "order_id": remoteID}
        path = "/api/futures/v3/cancel_order/%s/%s" % (symbol, remoteID)
        self.addRequest("POST", path, callback=self.onCancelOrder, data=req)

    # ----------------------------------------------------------------------
    def queryContract(self):
        """"""
        self.addRequest(
            "GET", "/api/futures/v3/instruments", callback=self.onQueryContract
        )

    # ----------------------------------------------------------------------
    def queryAccount(self):
        """"""
        self.addRequest("GET", "/api/futures/v3/accounts", callback=self.onQueryAccount)

    # ----------------------------------------------------------------------
    def queryPosition(self):
        """"""
        self.addRequest(
            "GET", "/api/futures/v3/position", callback=self.onQueryPosition
        )
    ...
    def queryOrder(self):
        ...
        for symbol in self.contractDict.keys():
            # 未成交
            req = {"instrument_id": symbol, "status": 0}
            path = "/api/futures/v3/orders/%s" % symbol
            self.addRequest("GET", path, params=req, callback=self.onQueryOrder)

            # 部分成交
            req2 = {"instrument_id": symbol, "status": 1}
            self.addRequest("GET", path, params=req2, callback=self.onQueryOrder)
    ...
    def onQueryPosition(self, data, request):
        ...
        for d in data["holding"][0]:
            ...
            longPosition.vtSymbol = ".".join(
                [longPosition.symbol, longPosition.exchange]
            )
            longPosition.direction = DIRECTION_LONG
            longPosition.vtPositionName = ".".join(
                [longPosition.vtSymbol, longPosition.direction]
            )
            ...
            shortPosition.vtPositionName = ".".join(
                [shortPosition.vtSymbol, shortPosition.direction]
            )
...
class OkexfWebsocketApi(WebsocketClient):
    ...
    def subscribe(self, subscribeReq):
        ...
        req1 = {"event": "addChannel", "channel": channel1}
        ...
        req2 = {"event": "addChannel", "channel": channel2}
    ...
    def onLogin(self, d):
        ...
        self.sendPacket(
            {"event": "addChannel", "channel": "ok_sub_futureusd_positions"}
        )
