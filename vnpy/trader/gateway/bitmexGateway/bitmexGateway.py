...
from vnpy.trader.vtConstant import EXCHANGE_BITMEX,PRODUCT_FUTURES
from vnpy.trader.vtConstant import STATUS_NOTTRADED,STATUS_PARTTRADED,STATUS_ALLTRADED,STATUS_CANCELLED,STATUS_REJECTED,STATUS_UNKNOWN
from vnpy.trader.vtConstant import DIRECTION_LONG,DIRECTION_SHORT,DIRECTION_NET,PRICETYPE_LIMITPRICE,PRICETYPE_MARKETPRICE
...
class BitmexRestApi(RestClient):
    ...
    def sign(self, request):
        ...
        if request.params:
            ...
            # 更新request得请求参数，重新排序
            request.params = OrderedDict(sorted(request.params.items()))
        else:
            path = request.path
    ...
    def connect(self, apiKey, apiSecret, sessionCount, testnet):
        ...
        self.writeLog(u'Bitmex REST API启动成功')
    ...
    def sendOrder(self, orderReq):   # type: (VtOrderReq)->str
        ...
        if orderReq.symbol.endswith('.{}'.format(EXCHANGE_BITMEX)):
            orderReq.symbol = orderReq.symbol.replace('.{}'.format(EXCHANGE_BITMEX),'')
        data = {
    ...
    #----------------------------------------------------------------------
    def onSendOrderFailed(self, _, *args):
        """
        下单失败回调：服务器明确告知下单失败
        """
        request, err_msg = None,None
        if len(args)>=1:
            request = args[0]
        if len(args) >= 2:
            err_msg = args[1]
        ...
        self.gateway.writeLog(u'requst header:{}'.format(request.headers))
        self.gateway.writeLog(u'requst method:{}'.format(request.method))
        self.gateway.writeLog(u'requst path:{}'.format(request.path))
        self.gateway.writeLog(u'requst params:{}'.format(request.params))
        self.gateway.writeLog(u'requst data:{}'.format(request.data))
        self.gateway.writeError(u'委托失败:{}'.format(err_msg))
...
class BitmexWebsocketApi(WebsocketClient):
    ...
    def authenticate(self):
        ...
        signature = hmac.new(self.apiSecret.encode('utf-8'), msg.encode('utf-8'), digestmod=hashlib.sha256).hexdigest()
    ...
    def onTick(self, d):
        ...
        utc_str_time = str(d.get('timestamp')).replace('T', ' ').replace('Z', '')
        utc_datetime = datetime.strptime(utc_str_time, '%Y-%m-%d %H:%M:%S.%f')
        tick.datetime = utc_datetime + timedelta(hours=8)
    ...
    def onTrade(self, d):
        ...
        utc_str_time = str(d.get('timestamp')).replace('T', ' ').replace('Z', '')
        utc_datetime = datetime.strptime(utc_str_time, '%Y-%m-%d %H:%M:%S.%f')
        trade.tradeTime = utc_datetime + timedelta(hours=8)
    ...
    def onOrder(self, d):
        ...
        if sysID in self.orderDict:
            ...
            utc_str_time = str(d.get('timestamp')).replace('T', ' ').replace('Z', '')
            utc_datetime = datetime.strptime(utc_str_time, '%Y-%m-%d %H:%M:%S.%f')
            order.orderTime = utc_datetime + timedelta(hours=8)
    ...
    def onPosition(self, d):
        ...
        pos.position = d.get('currentQty',0)
