...
class Api(FxcmApi):
    ...
    def processContracts(self, d):
        ...
        contract.productClass = productMapReverse.get(
            d["instrumentType"], PRODUCT_UNKNOWN
        )
    ...
    def sendOrder(self, orderReq):
        ...
        if orderReq.priceType == PRICETYPE_MARKETPRICE:
            reqid = self.openTrade(
                accountID, symbol, isBuy, amount, 1, "AtMarket", "DAY"
            )
        else:
            reqid = self.createEntryOrder(
                accountID, symbol, isBuy, rate, amount, "AtMarket", "DAY", limit=1
            )
