...
from vnpy.trader.vtConstant import STATUS_UNKNOWN,STATUS_NOTTRADED,STATUS_PARTTRADED,STATUS_CANCELLED,STATUS_ALLTRADED
from vnpy.trader.vtConstant import EXCHANGE_HUOBI,DIRECTION_LONG,PRODUCT_SPOT,DIRECTION_SHORT,OFFSET_NONE
...
class HuobiGateway(VtGateway):
    ...
    def connect(self):
        ...
        try:
            ...
            self.DEBUG = setting.get('trace', False)
            # 若希望连接后自动订阅
        ...
        if self.DEBUG:
            self.tradeApi.DEBUG = self.DEBUG
            self.dataApi.DEBUG = self.DEBUG

        for symbol_pair in self.auto_subscribe_symbol_pairs:
            self.writeLog(u'自动订阅现货合约:{}'.format(symbol_pair))
            req = VtSubscribeReq()
            req.symbol = symbol_pair
            req.exchange = EXCHANGE_HUOBI
            self.subscribe(req)
    ...
    def initQuery(self):
        ...
        if self.qryEnabled:
            # 需要循环的查询函数列表
            self.qryFunctionList = [self.tradeApi.qryOrder, self.tradeApi.qryTrade,self.tradeApi.qryPosition]
...
class HuobiTradeApi(TradeApi):
    ...
    def onGetAccounts(self, data, reqid):
        ...
        for d in data:
            ...
            account.vtAccountID = '.'.join([EXCHANGE_HUOBI,self.accountid])
            account.balance = 0.0
    ...
    def onGetMatchResult(self, data, reqid):
        """查询单一成交回调"""
        print('onGetMatchResult:{} {}'.format(reqid, data))
    ...
    def onBatchCancel(self, data, reqid):
        """批量撤单回调"""
        print('onBatchCancel:{} {}'.format(reqid, data))
