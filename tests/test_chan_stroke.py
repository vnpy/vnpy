...
def test_build_strokes_keeps_existing_same_side_when_new_fractal_is_weaker() -> None:
    fractals = [
        _fractal(0, FractalType.BOTTOM, 1, 9),
        _fractal(1, FractalType.BOTTOM, 3, 10),
        _fractal(2, FractalType.TOP, 5, 13),
    ]
...
def test_build_strokes_allows_loose_price_relationship_when_configured() -> None:
    fractals = [
        _fractal(0, FractalType.BOTTOM, 1, 9),
        _fractal(1, FractalType.TOP, 3, 8.5),
    ]

    strokes = build_strokes(fractals, ChanConfig(strict_stroke=False))

    assert len(strokes) == 1
    assert strokes[0].direction is ChanDirection.UP
