                ...
                with open(unicode(path), "wb") as f:
                    ...
                    for row in range(self.rowCount()):
                        ...
                        for column in range(self.columnCount()):
                            ...
                            if item is not None:
                                rowdata.append(text_type(item.text()).encode("gbk"))
                            else:
                                rowdata.append("")
...
class DepthMonitor(QtWidgets.QTableWidget):
    ...
    def updateTick(self, tick):
        ...
        self.updateCell(
            "bidValue1",
            tick.bidPrice1 * tick.bidVolume1 * self.contractSize,
            valueDecimals,
            data=tick.bidPrice1,
        )
        self.updateCell(
            "bidValue2",
            tick.bidPrice2 * tick.bidVolume2 * self.contractSize,
            valueDecimals,
            data=tick.bidPrice2,
        )
        self.updateCell(
            "bidValue3",
            tick.bidPrice3 * tick.bidVolume3 * self.contractSize,
            valueDecimals,
            data=tick.bidPrice3,
        )
        self.updateCell(
            "bidValue4",
            tick.bidPrice4 * tick.bidVolume4 * self.contractSize,
            valueDecimals,
            data=tick.bidPrice4,
        )
        self.updateCell(
            "bidValue5",
            tick.bidPrice5 * tick.bidVolume5 * self.contractSize,
            valueDecimals,
            data=tick.bidPrice5,
        )
        ...
        self.updateCell(
            "askValue1",
            tick.askPrice1 * tick.askVolume1 * self.contractSize,
            valueDecimals,
            data=tick.askPrice1,
        )
        self.updateCell(
            "askValue2",
            tick.askPrice2 * tick.askVolume2 * self.contractSize,
            valueDecimals,
            data=tick.askPrice2,
        )
        self.updateCell(
            "askValue3",
            tick.askPrice3 * tick.askVolume3 * self.contractSize,
            valueDecimals,
            data=tick.askPrice3,
        )
        self.updateCell(
            "askValue4",
            tick.askPrice4 * tick.askVolume4 * self.contractSize,
            valueDecimals,
            data=tick.askPrice4,
        )
        self.updateCell(
            "askValue5",
            tick.askPrice5 * tick.askVolume5 * self.contractSize,
            valueDecimals,
            data=tick.askPrice5,
        )
...
class TradingWidget(QtWidgets.QFrame):
    ...
    priceTypeList = [PRICETYPE_LIMITPRICE, PRICETYPE_MARKETPRICE]

    offsetList = [OFFSET_OPEN, OFFSET_CLOSE]
