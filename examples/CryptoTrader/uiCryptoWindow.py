...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def initCentral(self):
        ...
        widgetTradingW, dockTradingW = self.createDock(
            TradingWidget, vtText.TRADING, QtCore.Qt.RightDockWidgetArea
        )
        widgetMarketM, dockMarketM = self.createDock(
            MarketMonitor, vtText.MARKET_DATA, QtCore.Qt.LeftDockWidgetArea
        )

        widgetOrderM, dockOrderM = self.createDock(
            OrderMonitor, vtText.ORDER, QtCore.Qt.LeftDockWidgetArea
        )
        widgetWorkingOrderM, dockWorkingOrderM = self.createDock(
            WorkingOrderMonitor, vtText.WORKING_ORDER, QtCore.Qt.LeftDockWidgetArea
        )
        widgetTradeM, dockTradeM = self.createDock(
            TradeMonitor, vtText.TRADE, QtCore.Qt.LeftDockWidgetArea
        )

        widgetAccountM, dockAccountM = self.createDock(
            AccountMonitor, vtText.ACCOUNT, QtCore.Qt.RightDockWidgetArea
        )
        widgetPositionM, dockPositionM = self.createDock(
            PositionMonitor, vtText.POSITION, QtCore.Qt.RightDockWidgetArea
        )
        widgetLogM, dockLogM = self.createDock(
            LogMonitor, vtText.LOG, QtCore.Qt.RightDockWidgetArea
        )
