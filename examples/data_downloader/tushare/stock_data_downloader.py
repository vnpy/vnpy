...
for instrument in stockList:
    ...
    i = 0
    while i < 5:
        print(f"尝试第{i+1}次下载数据")
        if os.path.exists("./stock_data/%s.csv" % instrument[0]):
            currTime = time.time()
            lastModifiedTime = os.path.getmtime("./stock_data/%s.csv" % instrument[0])
            if os.path.getsize("./stock_data/%s.csv" % instrument[0]) / 1024 < 100 or \
                    currTime - lastModifiedTime > 120 * 60:
                df = ts.pro_bar(ts_code=instrument[0], adj='qfq', start_date=startDate, end_date=endDate)
                df.to_csv('./stock_data/%s.csv' % instrument[0])
                print(df)
            else:
                print(f"股票 {instrument[0]} 数据下载完成")
                break
        else:
            df = ts.pro_bar(ts_code=instrument[0], adj='qfq', start_date=startDate, end_date=endDate)
            df.to_csv('./stock_data/%s.csv' % instrument[0])
