...
class FixClient(fix.Application):
    ...
    def fromAdmin(self, msg, session_id):
        ...
        if d["<35>MsgType"] != "HEARTBEAT":
            ...
            if d.get("<58>Text"):
                if d["<58>Text"].startswith("MsgSeqNum too"):
                    self.acceptor_respond(d)
            # 
            if d["<35>MsgType"] == "LOGON":
                self.reset_seq(d)


    def reset_seq(self, d):
        print("start reset next msg seq number----------------")
        msg_num = d["<34>MsgSeqNum"]
        order = fix.Message()
        ...
        order.setField(789, msg_num)
        order.setField(34, msg_num)
        order.getHeader().setField(fix.MsgType(fix.MsgType_Logon))
        self.put_order(self.session_id, order)



    def acceptor_respond(self, data):
        print("@@@@@@@@@---------acceptor_respond")
        msg_num = data["<58>Text"].split(" ")[-4]
        order = fix.Message()
        order.getHeader().setField(fix.StringField(8, "FIX.4.2"))
        # order.getHeader().setField(fix.MsgType(fix.MsgType_Logon))  # MsgType
    ...
    def msg2dict(self, msg) -> dict:
        ...
        if len(words) == 1 and words[0] == 'FIX.4.2:VnpyUAT6->GenusVnpyMarket':
            return 'FIX.4.2:VnpyUAT6->GenusVnpyMarket'
