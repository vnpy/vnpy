...
到这一步，流程已经从因子表推进到模型预测值。接下来要处理的问题是：**如何把预测值变成每日可执行的策略规则？**

模型预测只是多因子截面策略中的一环，它回答“哪些股票在横截面上更值得看好”；策略开发继续向前推进，回答“怎样根据这些分数形成持仓、控制换手，并在每天的行情到来时做出调仓动作”。

本篇进入 **`AlphaStrategy` 策略开发**，重点说明三件事：
...
在多因子截面策略开发中，模型阶段和策略阶段经常连在一起运行，但关注点各有侧重。
...
模型输出的是一组**预测分数**，策略输出的是一组**目标持仓**。同一组信号，在 `top_k`、换仓速度、资金分配规则变化后，回测结果也会随之变化。
...
这里的关键是顺序一致：`pred` 的顺序应当和 `infer_df` 的行顺序一致。官方模型内部预测前会按 `datetime`、`vt_symbol` 排序，生成信号表时也应使用同一段推理数据，避免日期或标的错位。
...
它们仍然属于**信号视角**。信号分析通常不处理持仓数量、换仓次数、交易费用、无法成交、现金比例等问题；这些因素会在策略和回测阶段影响真实组合表现。

可以把信号评价看作进入策略开发前的一道检查：
...
`vnpy.alpha` 中的策略基类是 `AlphaStrategy`。模型负责拟合参数，策略负责定义回测过程中的响应方式。
...
这一层的核心职责可以概括为：**`AlphaStrategy` 负责把“当日信号”翻译成“目标仓位”。**
...
策略通过 `self.get_signal()` 向回测引擎取出当前交易日的信号。回测引擎内部会按当前 `datetime` 从 `signal_df` 中过滤当日行。信号表日期需要和回测行情日期对齐，缺少当日信号时会出现找不到信号的日志。
...
卖出前还会检查 `min_days`。持仓天数不足的股票即使进入卖出列表，也会暂时跳过，用来控制过于频繁的买卖。
...
这里的直觉是：当前持仓少于 `top_k` 时补足；本次卖出了一些股票时，用新的高分股票替换。买入标的来自未持仓股票中信号排名靠前的部分。
...
这些参数属于**组合构建和交易执行假设**。同一组预测信号，在 `top_k`、换仓速度、费用设置变化后，回测结果也会随之变化。模型预测指标之外，还需要单独做策略层验证。
