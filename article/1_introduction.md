...
已经会使用 Python、也做过一些截面因子或机器学习实验的读者，通常会先关心这几个问题：

1. 行情数据和指数成分，应该如何组织起来？
2. 因子、标签、模型、回测，各自负责哪一段？
3. 官方有没有一套可以直接照着跑、再逐步替换的研究范例？

围绕这些问题，我们准备写一个 `vnpy.alpha` 入门系列，按投研流程把整个模块串起来：从行情与指数成分入库，到因子与标签表构建，再到模型训练、信号生成和组合回测，逐步带大家走完整条研究链路。
...
它面向**投研端**场景，重点放在围绕一批股票构建因子、训练模型、输出预测信号，并用历史数据验证策略表现。交易接入、实盘委托和界面交互，则属于 VeighNa 主程序和交易网关侧的能力。
...
这张图可以帮助读者先建立一个整体印象：**`vnpy.alpha` 覆盖从数据准备到策略验证的一条完整投研链路。**
...
内置的 Alpha 158、Alpha 101 等因子集，也都属于这一层的能力范围。它们可以帮助研究流程先快速建立一条可运行、可对照的基线，后续再逐步替换为自己的因子。
...
这一篇先帮助读者建立 `vnpy.alpha` 的全貌。细节部分会在后续文章中沿着投研流程逐步展开。
...
这些组件围绕**“数据 -> 因子 -> 模型 -> 信号 -> 回测”**组织在一起，形成一套完整的研究框架。
