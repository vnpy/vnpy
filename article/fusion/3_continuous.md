...
本篇只讲清这件事：为什么要连续合约、指数 / 前复权 / 后复权各自适合什么场景、为什么不同软件的数据常对不上。复权只给必要公式，不写证明。
...
如果需要进一步理解它们的数学关系，记 \(t\) 为当前观察日，\(t_k\) 为第 \(k\) 次换月的生效日，换月价差定义为 \(\mathrm{Gap}_k=\text{新主力价格}-\text{旧主力价格}\)。各家采用开盘价还是收盘价计算，规则可能不同。
