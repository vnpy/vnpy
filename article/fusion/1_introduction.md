...
| 投研数据 | 需自行购买并配置datafeed服务，如米筐RQData等 | 内置数据中心，支持本地期货连续数据合成 |
| 实盘接入 | 须先完成系统合规测试，再申请程序化交易权限 | 免去繁琐测试步骤，开通权限即可直接使用 |

这两条路径面向的是不同类型用户的需求：VeighNa 开源版适合希望自行配置环境、扩展功能的开发者；VeighNa Fusion适合希望把精力放在量化投研与交易本身的交易者。
...
下面两张图分别是Fusion主界面，以及内置智策工作台（智策投研）的工作流全景——后文演示会沿着这条工作流展开。
