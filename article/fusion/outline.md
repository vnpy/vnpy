...
| 4 | 已定稿 | `article/fusion/4_code.md` |
| 5～8 | 未成稿；须遵守本大纲与第 2、4 篇确认稿基线 | — |
...
**钩子句**：你不需要会写代码，但你必须看得懂自己的资金按什么规则在交易。

**发布标题**：【CTA量化通关系列4 - 不会写代码，也要会读策略】

**成稿**：`article/fusion/4_code.md`（已定稿；与统一实验产物 `new_boll_strategy.py` 对齐）

**章节展开（约 2800 字）**：

1. **开篇**：承接第 3 篇；写代码可借助工具，读代码与确认责任不能外包（呼应第 2 篇钩子）。
2. **规则落在代码的哪五处（本篇核心；即五个固定部件）**：对照第 2 篇确认稿 / 说明书与真实生成代码（标准 `CtaTemplate` 模块化辅助函数结构）逐块讲解；节首配【配图 A：规则五要素在代码中的落点对照】（源码见第十一节）。讲解落点，不要求「部件与要素机械一一对应」。开篇须有「本篇只做一件事 + 不教 Python、不逐行讲完」；落点须先定义再使用：
   - 参数与变量声明 → 可调旋钮（`parameters` 包含信号与周期参数，`fixed_size` 可配置但在第 7 篇优化时保持固定）；
   - `BarGenerator` → 信号周期（1m→15m，`on_bar` 仅推入合成器不发单）；
   - `ArrayManager` 与 `calculate_indicators` → 指标数值（布林/Aroon/ATR 计算与预热保护）；
   - 信号判断与下单 `handle_no_position` → 入场（`cancel_all` 撤旧单 + Aroon 过滤 + 布林轨道 `stop=True` 停止单）；
   - 持仓管理与止损 `update_long_position` / `update_short_position` 与 `on_trade` → 出场（`max/min` 移动止损 + 停止单平仓；极值在 `on_trade` 开仓成交时用 `trade.price` 初始化、平仓重置为 0.0）。
   - 教学主线：**规则五要素在代码中的落点对照**——读代码 = 核对翻译是否完整还原。
3. **核对示范**：拿说明书核对「1.5×ATR 移动止损」落点（参数声明、`calculate_indicators`、`on_trade` 成交初始化、`update_long_position` 极值跟踪与 `cancel_all`）。收束回到第 1 篇：这些点对不上，常见走样是移动止损变成固定止损。**不要**把 ATR 随波动变化导致止损价松紧判成翻译偏差——移动止损的硬约束是极值只顺势更新。
4. **Fusion 演示段（C 类 2 张）**：【配图 C：智策投研「策略代码」页】（生成后流程可能自动往下走，正文按此改口，勿写成「停在生成代码」）、【配图 C：打开策略文件后的参数段】（可用编辑器视图，须写明从智策「打开策略文件」进入）；说明产物为标准 `CtaTemplate`，与开源版通用。止损倍数名以核对底稿为准（`sl_multiplier`），图中若为 `atr_stop_multiplier` 须点明对的是 1.5。
5. **小结 + 预告**：预告第 5 篇典型错误与审查清单。

**不写什么**：不逐行讲完全部代码；不教 Python 语法；不展开全部回调。

**执行注意**：代码截取必须来自第七节统一实验档案中的真实产物；每段 ≤15 行。逻辑基线以第 2 篇确认稿及 `new_boll_strategy.py` 为准，不得另起炉灶。
...
1. **开篇（约 300 字）**：承接第 4 篇核对示范（能跑完 ≠ 翻译对；常见走样是移动止损变固定止损）；引用行业共识与 Fusion 用户协议的表述——AI 输出可能存在错误、遗漏、偏差，必须独立审查验证后才能用于实盘。把合规条款转述为知识点，树立负责任形象。勿再用「ATR 放大时止损价下移」当偏差例。
...
| 初始资金 | 全系列统一（写作前锁定） |
| parameters 声明列表 | `boll_window`, `boll_dev`, `aroon_window`, `atr_window`, `sl_multiplier`, `fixed_size`, `interval`（实例可配置；`fixed_size` 与 `interval` 在优化时固定） |
| 第 7 篇优化参数及范围 | 列出拟优化参数名、搜索范围与步长（≤3 个关键信号/过滤参数，如 `boll_window`, `boll_dev`, `sl_multiplier`）；与说明书参数名一致 |
...
- 智策逻辑与代码产物结构：智策投研在生成逻辑产物中包含完整步骤、`parameters` / `variables` 列表，以及 `assumptions`（假设列表，如成交价初始化极值、平仓归零、无持仓最小假设）与 `issues`（边界问题列表，如 Aroon 严格相等概率极低、极端跳空偏差），是第 4/5 篇代码审查与边界分析的真实素材来源
