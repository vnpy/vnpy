...
{
	//该函数为手动编写
	THOST_TE_RESUME_TYPE type = THOST_TE_RESUME_TYPE(nType);
...
int TdApi::reqMBLQuot(dict req, int nRequestID)
{
	...
	memset(&myreq, 0, sizeof(myreq));
	getStr(req, "EndInstrumentID", myreq.EndInstrumentID);
	getStr(req, "StartInstrumentID", myreq.StartInstrumentID);
	getChar(req, "Direction", &myreq.Direction);
	int i = this->api->ReqMBLQuot(&myreq, nRequestID);
	return i;
};
