...
RUN sed -i 's/archive.ubuntu.com/mirrors.tuna.tsinghua.edu.cn/g' /etc/apt/sources.list && \
    ...
    apt-get install -y --no-install-recommends wget vim build-essential python3 python3-dev pip \
...
RUN pip install --no-cache vnpy vnpy-algotrading vnpy-chartwizard vnpy-ctabacktester vnpy-ctastrategy vnpy-ctp vnpy-ctptest \
    vnpy-datamanager vnpy-datarecorder vnpy-esunny vnpy-ib vnpy-mini vnpy-mongodb vnpy-mysql \
    vnpy-optionmaster vnpy-paperaccount vnpy-portfoliomanager vnpy-portfoliostrategy vnpy-postgresql vnpy-riskmanager \
    vnpy-rohon vnpy-rpcservice vnpy-rqdata vnpy-scripttrader \
    vnpy-sopt vnpy-spreadtrading vnpy-sqlite vnpy-tap vnpy-tora vnpy-tts vnpy-tushare vnpy-uft vnpy-webtrader vnpy-xtp

#沿用danchitnis/xrdp:ubuntu-xfce ENTRYPOINT
