...
Interactive Brokers盈透证券（IB）提供丰富的外盘市场历史数据下载服务（包括股票、期货、期权、现货等），注意下载前需要先启动IB TWS交易软件，并在VeighNa Trader主界面连接好IB接口，并订阅所需合约行情。
...
点击【确定】按钮，开始从CSV文件导入数据到数据库中。在导入过程中，界面可能会出现暂时无响应的状态，这种状态的持续时间取决于CSV文件的大小（即数据量）。数据成功导入后，将出现弹窗显示数据载入成功，如下图所示：
...
在数据库已有数据的前提下，点击表格最左侧【数据】列下数据频率前的小箭头，则可展开或收起该数据频率下的合约信息显示。
...
如果依赖于交易接口提供的历史数据，一旦开始时间和结束时间之间的时间跨度超过接口所能提供的数据范围，可能会出现数据缺失的情况。因此建议更新数据之后，点击【查看】按钮检查该合约数据是否连续。