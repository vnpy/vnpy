...
class PublicAPI:
    def __init__(self, production=True, version="v1", timeout=20):
        self.__host = (
            production
            and "https://api.korbit.co.kr/%s/" % version
    ...
    def orderbook(self, currency_pair="btc_krw", category="all", group=True):
        params = {"group": group, "category": category, "currency_pair": currency_pair}
    ...
    def list_of_filled_orders(self, currency_pair="btc_krw", interval="hour"):
        params = {"time": interval, "currency_pair": currency_pair}
    ...
    def request_post(self, path, headers=None, data=None):
        ...
        response = requests.post(
            urljoin(self.host, path), headers=headers, data=data, timeout=self.__timeout
        )
        print(response)
        try:
            return response.json()
