...
class PrivateAPI(PublicAPI):
    ...
    def headers(self):
        return {
            ...
            "Authorization": "{} {}".format(
                self.__token["token_type"], self.__token["access_token"]
            ),
        }
    ...
    def market_bid_order(self, fiat_amount, currency_pair="btc_krw"):
        return self.bid_order(
            "market", fiat_amount=fiat_amount, currency_pair=currency_pair
        )

    def limit_bid_order(self, coin_amount, price, currency_pair="btc_krw"):
        return self.bid_order(
            "limit", coin_amount=coin_amount, price=price, currency_pair=currency_pair
        )
    ...
    def market_ask_order(self, coin_amount, currency_pair="btc_krw"):
        return self.ask_order(
            "market", coin_amount=coin_amount, currency_pair=currency_pair
        )
    ...
    def cancel_order(self, ids, currency_pair="btc_krw"):
        payload = {"id": ids, "currency_pair": currency_pair, "nonce": self.nonce}
        return self.request_post(
            "user/orders/cancel", headers=self.headers, data=payload
        )

    def list_open_orders(self, offset=0, limit=10, currency_pair="btc_krw"):
        params = {"currency_pair": currency_pair, "offset": offset, "limit": limit}
    ...
    def view_transfers(self, offset=0, limit=10, currency="btc"):
        params = {"currency": currency, "offset": offset, "limit": limit}
    ...
    def assign_btc_address(self, currency="btc"):
        payload = {"currency": currency, "nonce": self.nonce}
        return self.request_post(
            "user/coins/address/assign", headers=self.headers, data=payload
        )
    ...
    def status_of_btc_deposit_and_transfer(self, transfer_id="", currency="btc"):
        ...
        return self.request_get(
            "user/coins/status", headers=self.headers, params=params
        )

    def cancel_btc_transfer_request(self, transfer_id, currency="btc"):
        payload = {"id": transfer_id, "currency": currency, "nonce": self.nonce}
        return self.request_post(
            "user/coins/out/cancel", headers=self.headers, data=payload
        )
