...
class TradeApi(object):
    ...
    def make_header(self, url, body=""):
        ...
        signature = hmac.new(
            self.secretKey.encode("utf-8"), message.encode("utf-8"), hashlib.sha256
        ).hexdigest()
        headers = {
    ...
    def processRequest(self, req):
        ...
        if method in [
            FUNCTIONCODE_GET_INFO_COINCHECK,
            FUNCTIONCODE_GET_BALANCE_COINCHECK,
            FUNCTIONCODE_LIST_ORDER_COINCHECK,
            FUNCTIONCODE_HISTORY_ORDER_COINCHECK,
        ]:
            r = requests.get(url, headers=headers)
        elif method in [FUNCTIONCODE_CANCEL_ORDER_COINCHECK]:
            r = requests.delete(url, headers=headers)
        elif method in [
            FUNCTIONCODE_BUY_ORDER_COINCHECK,
            FUNCTIONCODE_SELL_ORDER_COINCHECK,
        ]:
            kwargs = req["kwargs"]
            payload = {
                "rate": kwargs["rate"],
            ...
            body = "rate={rate}&amount={amount}&order_type={order_type}&pair={pair}".format(
                **payload
            )
    ...
    def get_info(self):
        return self.sendRequest(
            self.API_account, FUNCTIONCODE_GET_INFO_COINCHECK, self.onGet_info, None
        )
    ...
    def cancel_orders(self, order_id):
        return self.sendRequest(
            self.API_cancel_order % (str(order_id)),
            FUNCTIONCODE_CANCEL_ORDER_COINCHECK,
            self.onCancel_orders,
            None,
        )

    def history_orders(self):
        return self.sendRequest(
            self.API_history_order,
            FUNCTIONCODE_HISTORY_ORDER_COINCHECK,
            self.onHistory_orders,
            None,
        )
...
class DataApi(object):
    ...
    TICK_SYMBOL_URL = {SYMBOL_BTCJPY: "https://coincheck.com/api/ticker"}
    """get latest deal history of coincheck market"""
    TRADES_SYMBOL_URL = {SYMBOL_BTCJPY: "https://coincheck.com/api/trades"}
    """get latest asks/bids information of coincheck market"""
    ORDERBOOKERS_SYMBOL_URL = {SYMBOL_BTCJPY: "https://coincheck.com/api/order_books"}
