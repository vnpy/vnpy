...
SCHEMA = {
    ...
    "currency": {"required": True, "type": "string", "allowed": ["jpy", "btc", "mona"]},
...
class TradeApi(object):
    ...
    def __get_header(self, params):
        signature = hmac.new(
            bytearray(self.secretKey.encode("utf-8")), digestmod=hashlib.sha512
        )
        signature.update(params.encode("utf-8"))

        return {"key": str(self.accessKey), "sign": str(signature.hexdigest())}
            ...
            for url, callback in self.taskList:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        data = {
                            "return_symbol": (url.split("/"))[-1].split("_")[0],
                            "data": data,
                        }
                        # data["return_symbol"] =
                        callback(data)
