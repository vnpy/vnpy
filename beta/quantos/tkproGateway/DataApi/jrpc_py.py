...
class JRpcClient(object):
    ...
    def _recv_run(self):
        ...
        while not self._should_close:
            ...
            try:
                if (
                    self._connected
                    and time.time() - self._last_heartbeat_rsp_time
                    > self._heartbeat_timeout
                ):
                    self._connected = False
                    if self.on_disconnected:
                        self._async_call(self.on_disconnected)

                if (
                    remote_sock
                    and time.time() - heartbeat_time > self._heartbeat_interval
                ):
                    self._send_hearbeat()
                ...
                if (
                    remote_sock
                    and remote_sock in socks
                    and socks[remote_sock] == zmq.POLLIN
                ):
        ...
        try:
            ...
            if "method" in msg and msg["method"] == ".sys.heartbeat":
                ...
                if "result" in msg and self.on_rpc_callback:
                    self._async_call(
                        lambda: self.on_rpc_callback(msg["method"], msg["result"])
                    )
    ...
    def call(self, method, params, timeout=6):
        ...
        msg = {"jsonrpc": "2.0", "method": method, "params": params, "id": str(callid)}
