...
class TkproTradeApi(object):
    ...
    def onOrderStatus(self, data):
        ...
        order.direction, order.offset = actionMapReverse.get(
            data.entrust_action, (DIRECTION_UNKNOWN, OFFSET_UNKNOWN)
        )
    ...
    def onTrade(self, data):
        ...
        trade.direction, trade.offset = actionMapReverse.get(
            data.entrust_action, (DIRECTION_UNKNOWN, OFFSET_UNKNOWN)
        )
    ...
    def sendOrder(self, orderReq):
        ...
        taskid, msg = self.api.place_order(
            security, action, orderReq.price, int(orderReq.volume)
        )
...
class TkproDataApi(object):
    ...
    def subscribe(self, subscribeReq):
        ...
        subscribed, msg = self.api.subscribe(
            security, fields=self.fields, func=self.onMarketData
        )
