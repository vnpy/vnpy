...
class TradeApi(object):
    ...
    def confirm_internal_order(self, task_id, confirmed):
        ...
        rpc_params = {"task_id": task_id, "confirmed": confirmed}
    ...
    def place_order(
        self, security, action, price, size, algo="", algo_param={}, userdata=""
    ):
        """
        return (result, message)
        if result is None, message contains error information
    ...
    def batch_order(self, orders, algo="", algo_param={}, userdata=""):
        ...
        if isinstance(orders[0], EntrustOrder):
            ...
            for o in orders:
                tmp.append(
                    {"security": o.security, "price": o.price, "size": int(o.size)}
                )
    ...
    def query_repo_contract(self, format=""):
        ...
        return utils.extract_result(
            cr, format=self._get_format(format, "pandas"), class_name="RepoContract"
        )
    ...
    def query_portfolio(self, format=""):
        ...
        return utils.extract_result(
            cr, index_column="security", format=data_format, class_name="NetPosition"
        )
    ...
    def set_heartbeat(self, interval, timeout):
        ...
        print(
            "heartbeat_interval =",
            self._remote._heartbeat_interval,
            ", heartbeat_timeout =",
