...
class SniperAlgo(StAlgoTemplate):
    ...
    def updateSpreadTick(self, spread):
        ...
        # 若当前已有主动腿委托则直接返回
        if (
            self.activeVtSymbol in self.legOrderDict
            and self.legOrderDict[self.activeVtSymbol]
        ):
            return
        ...
        if self.mode == self.MODE_LONGSHORT or self.mode == self.MODE_LONGONLY:
            ...
            # 卖出
            elif spread.netPos > 0 and spread.bidPrice >= self.sellPrice:
        ...
        if self.mode == self.MODE_LONGSHORT or self.mode == self.MODE_SHORTONLY:
            ...
            # 平空
            elif spread.netPos < 0 and spread.askPrice <= self.coverPrice:
                self.quoteActiveLeg(self.SPREAD_LONG)
    ...
    def quoteActiveLeg(self, direction):
        ...
        if direction == self.SPREAD_LONG:
            spreadVolume = min(
                spread.askVolume, self.maxPosSize - spread.netPos, self.maxOrderSize
            )
            ...
            spreadVolume = min(
                spread.bidVolume, self.maxPosSize + spread.netPos, self.maxOrderSize
            )
