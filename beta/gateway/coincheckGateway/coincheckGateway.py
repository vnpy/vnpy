    ...
    def onGet_balance(self, data, req, reqID):
        if data["success"] == 0:
            ...
            for symbol in ["btc", "jpy"]:
                ...
                posObj.position = float(data[symbol]) + float(
                    data[symbol + "_reserved"]
                )
...
'id': 324141928, u'pending_amount': u'0.005'}], u'success': True}
    ...
    def onList_order(self, data, req, reqID):
        ...
        if data["success"] == 0:
            ...
            ten_seconds_str = (ten_seconds_before.strftime("%Y-%m-%dT%H:%M:%S.%f"))[
                :-3
            ] + "Z"
            ...
            for d in orders:
                ...
                if coinID in local_system_dict_keys:
                    ...
                    if order != None:
                        ...
                        newTradeVolume = float(has_traded_volume) - float(
                            bef_has_volume
                        )
                        ...
                        if newTradeVolume > 0.00000001:
                            ...
                            trade.vtTradeID = ".".join(
                                [trade.tradeID, trade.gatewayName]
                            )
                            trade.orderID = order.orderID
                            trade.vtOrderID = order.vtOrderID
