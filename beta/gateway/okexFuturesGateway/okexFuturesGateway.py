...
class OkexFuturesGateway(VtGateway):
    ...
    def subscribeUserTrade(self):
        self.webSocket.sendPacket(
            {"event": "addChannel", "channel": "ok_sub_futureusd_trades"}
        )
    ...
    def _generateLocalOrder(self, symbol, price, volume, direction, offset):
        ...
        myorder.vtOrder = VtOrderData.createFromGateway(
            self, localId, symbol, self.exchange, price, volume, direction, offset
        )
        return myorder
    ...
    def sendOrder(self, vtRequest):  # type: (VtOrderReq)->str
        ...
        myorder = self._generateLocalOrder(
            symbol,
            vtRequest.price,
            vtRequest.volume,
            vtRequest.direction,
        ...
        orderType = _orderTypeMap[
            (vtRequest.direction, vtRequest.offset)
        ]  # 开多、开空、平多、平空
    ...
    def cancelOrder(self, vtCancel):  # type: (VtCancelOrderReq)->None
        ...
        self.restClient.addRequest(
            ...
            data={"instrument_id": symbol, "order_id": remoteId},
            extra=myorder,
        )

    # ----------------------------------------------------------------------
    def queryContracts(self):
        return self.restClient.addRequest(
            "GET", "/api/futures/v3/instruments", callback=self._onQueryContracts
        )
    ...
    def qryAccount(self):
        ...
        """
        return self.restClient.addRequest(
            "GET", "/api/futures/v3/accounts", callback=self._onQryAccounts
        )
    ...
    def qryPosition(self):
        """查询持仓"""
        return self.restClient.addRequest(
            "GET", "/api/futures/v3/position", callback=self._onQueryPosition
        )
    ...
    def _onQueryContracts(self, data, _):  # type: (dict, Request)->None
        ...
        for easySymbol, sortedSymbols in s2.items():
            if len(sortedSymbols) == 3:
                for contractType, v3symbol in zip(
                    _contractTypeMap.keys(), sortedSymbols
                ):
                    uiSymbol = "{}_USD_{}".format(
                        easySymbol, contractType
                    )  # ETC_USD_THISWEEK
                    remoteContractName = "{}{}".format(
                        easySymbol, v3symbol[-4:]
                    )  # ETC1201
        ...
        for contract in data:
            symbol = contract["instrument_id"]
            size = (
                contract["quote_increment"]
                if "quote_increment" in contract
                else contract["trade_increment"],
            )
        ...
        if data["result"] is True:
            for info in data["orders"]:
                ...
                if myorder:
                    ...
                    myorder = self._generateLocalOrder(
                        symbol, info["price"], info["size"], direction, offset
                    )
                    myorder.vtOrder.tradedVolume = tradedVolume
                    myorder.remoteId = remoteId
                    self._saveRemoteId(myorder.remoteId, myorder)
        ...
        for packet in packets:
            ...
            try:
                if channel.type == ChannelType.Tick:
                    uiSymbol = remoteSymbolToLocal(
                        channel.symbol,
                        remoteContractTypeToLocal(channel.remoteContractType),
                    )
                    if self._lastTicker is None:
                        ...
                        self._lastTicker.date = self._lastTicker.datetime.strftime(
                            "%Y%m%d"
                        )
                        self._lastTicker.time = self._lastTicker.datetime.strftime(
                            "%H:%M:%S"
                        )
                    ...
                    for pos in positions:
                        ...
                        self.onPosition(
                            VtPositionData.createFromGateway(
                                gateway=self,
                                exchange=self.exchange,
                                symbol=symbol,
                                direction=direction,
                    ...
                    if order:
                        self.onTrade(
                            VtTradeData.createFromOrderData(
                                order=order.vtOrder,
                                tradeID=tradeID,
    ...
    def onApiError(
        self, exceptionType, exceptionValue, tb, request=None  # type: Request
    ):
...
# ----------------------------------------------------------------------
def localSymbolToRemote(
    symbol
):  # type: (str)->(OkexFuturesSymbol, OkexFuturesContractType)
    """
...
_prefixForRemoteContractType = {
    v.split("_")[0]: v
    for k, v in OkexFuturesContractType.__dict__.items()
...
_symbolsForUi = {
    (remoteSymbol.upper() + "_" + upperContractType.upper()): (
        remoteSymbol,
        remoteContractType,
    )
    for remoteSymbol in _remoteSymbols
    for upperContractType, remoteContractType in _contractTypeMap.items()
...
_channel_for_subscribe = {
    ...
    + easySymbol
    + "_ticker_"
    + remoteContractType: (easySymbol, remoteContractType)
