    ...
    def initQuery(self):
        ...
        if self.qryEnabled:
            self.qryFunctionList = [
                self.tradeApi.list_market_all_register,
                self.tradeApi.user_balances,
            ]
            self.startQuery()
...
class KorbitTradeApi(vnkorbit.Korbit_TradeApi):
    ...
    def sendOrder(self, req):
        ...
        if req.direction == DIRECTION_LONG:
            reqID = self.buy_currency(
                coin_amount=req.volume, price=req.price, currency_pair=currency
            )
        else:
            reqID = self.sell_currency(
                coin_amount=req.volume, price=req.price, currency_pair=currency
            )
