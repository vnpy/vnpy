...
class EventEngine:
    ...
    def __init__(self):
        ...
        # 事件引擎开关
        self.__active = t_Event()

        # 事件处理线程
        self.__thread = Thread(target=self.__run)

        # 计时器，用于触发计时器事件
        self.__timer = RepeatTimer(1, self.__onTimer)
    ...
    def __run(self):
        ...
        while self.__active.is_set():
            event = self.__queue.get(block=True)  # 队列阻塞，超时1秒由repeatTimer进行心跳
    ...
    def register(self, type_, handler):
        ...
        if type_ not in self.__handlers.keys():
            self.__handlers[type_] = [handler]
        else:
    ...
    def unregister(self, type_, handler):
        ...
        if type_ in self.__handlers.keys() and handler in self.__handlers[type_]:
...
class RepeatTimer(Thread):
    """Call a function every specified number of seconds:
            ...
            t = RepeatTimer(30.0, f, args=[], kwargs={})
            t.start()
